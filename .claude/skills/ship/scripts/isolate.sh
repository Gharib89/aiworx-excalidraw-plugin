#!/usr/bin/env bash
# Ship phase-0 isolation when the EnterWorktree tool is absent: fresh worktree
# on a clean branch off the up-to-date default branch, with the runtime deps a
# worktree needs to run the tests (node_modules is not shared with the main
# checkout). Prints the worktree path last.
#
#   scripts/isolate.sh <type> <slug> <issue>     e.g. isolate.sh fix gate-overlap 42
set -u
TYPE="${1:?usage: isolate.sh <type> <slug> <issue>}"
SLUG="${2:?usage: isolate.sh <type> <slug> <issue>}"
N="${3:?usage: isolate.sh <type> <slug> <issue>}"
BRANCH="$TYPE/$SLUG-$N"

ROOT=$(git rev-parse --show-toplevel) || exit 1
git -C "$ROOT" fetch origin --quiet || {
  echo "git fetch failed — refusing to branch off a possibly-stale default" >&2
  exit 1
}
DEFAULT=$(git -C "$ROOT" symbolic-ref -q --short refs/remotes/origin/HEAD || echo origin/main)

git -C "$ROOT" rev-parse --verify -q "refs/heads/$BRANCH" >/dev/null && {
  echo "branch $BRANCH already exists — preflight should have stopped this run" >&2
  exit 1
}

WT="$ROOT/.claude/worktrees/$SLUG-$N"
mkdir -p "$ROOT/.claude/worktrees"
git -C "$ROOT" worktree add -b "$BRANCH" "$WT" "$DEFAULT" >/dev/null || exit 1

# Runtime deps (playwright-core) are all the test suite needs; the local gate
# upgrades to a full `npm ci` if the change touches a bundle input.
(cd "$WT" && npm ci --omit=dev >/dev/null 2>&1) || {
  echo "npm ci --omit=dev failed in the worktree" >&2
  exit 1
}
echo "$WT"
