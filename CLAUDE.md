...
- **`examples/`** — a band **draws** commands on canvas, so correcting one means regenerating the artifact rather than editing a file. A command or flag change lands in every band that draws it: grep the generators for it, then re-run each one whose text moved. `tests/drawn-commands.js` walks every committed band and holds its drawn flags to the CLIs' real inventories (`tools/cli-flags.js`) — a band left behind goes red in `test:fast`. Element identity is **derived** (`CONTEXT.md`, **Derived identity**), so re-running a generator is a no-op unless something moved: `tests/band-bytes.js` regenerates every band out-of-tree and refuses any byte difference. That makes `git diff examples/` the answer to "did my change reach a picture?" — and it means a change that legitimately moves geometry ships its regenerated artifacts in the same commit.
