...
- `gitleaks/gitleaks-action` (CI `secrets` leg): no credential in the pushed or PR commits; `scripts/local-gate.sh` runs the same scan over added lines before a PR opens.
