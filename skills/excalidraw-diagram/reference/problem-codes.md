...
| `uncovered-glyph` | live | presence | [text] | `codepoints`, `family` | a text draws a character the vendored woff2 files for its own `fontFamily` carry no glyph for, so the browser skips the house face and the machine's font supplies the glyph: the measured width is the machine's, not the plugin's. `codepoints` names each one as `U+2713`, once per character however often it is drawn, and `family` is the vendored family that lacks it. Coverage is read from each shipped woff2 file's own `cmap`, so it is what the bytes carry rather than what a range declares, and it is per family: the prose face carries `+`, `×`, `−`, `†` and `•` but neither the tick and cross marks (`✓` U+2713, `✗` U+2717) nor the arrows (`→` U+2192, `↑` U+2191, `↓` U+2193), while the code face carries the tick and the arrows and still not the cross. Prose is where a label's marks land, so treat the prose face's set as the one to draw from |
