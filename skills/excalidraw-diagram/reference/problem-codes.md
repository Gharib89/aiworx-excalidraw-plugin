...
| `uncovered-glyph` | live | presence | [text] | `codepoints`, `family` | a text draws a character the vendored woff2 files for its own `fontFamily` carry no glyph for, so the browser skips the house face and the machine's font supplies the glyph: the measured width is the machine's, not the plugin's. `codepoints` names each one as `U+2713`, once per character however often it is drawn, and `family` is the vendored family that lacks it. The tick and cross marks (`✓` U+2713, `✗` U+2717) and the arrows (`→` U+2192, `↑` U+2191, `↓` U+2193) are the common cases; `+`, `×`, `−`, `†` and `•` are carried. Coverage is read from the shipped bytes, so a diagram written before the check still scores it |
