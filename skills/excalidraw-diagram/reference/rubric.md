...
| 5 | Role hues stay under the ceiling, grey excluded; `pass` and `fail` are never told apart by hue alone — a second channel, a `+`/`×` mark or a stroke style, always rides with red and green. The mark has to be one the house fonts carry: a tick or a cross measures per-machine, which `uncovered-glyph` reports | `panel` | A + P | `too-many-hues`, `hue-only-pass-fail`, `uncovered-glyph` |
