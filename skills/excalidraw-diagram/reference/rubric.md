...
| 2 | The picture's aspect matches the surface it was authored for | `panel` | A + P | `aspect-off-preset` |
...
| 6 | Same element type, same size within a `graph` layer or a row — widths within 1.25× of each other. Measure the widest member first and size the rest to it | `picture` | P | — |
...
| 11 | Stroke widths run the 1:2:4 ladder against the depth tiers — leaders and arrows thinnest, the register in the middle, at most one focal element at the top per panel; at most three widths in a diagram | `panel` | P + A | `flat-stroke-weight` |
