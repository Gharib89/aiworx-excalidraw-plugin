...
| 5 | Role hues stay under the ceiling, grey excluded; `pass` and `fail` are never told apart by hue alone — a second channel, a `+`/`×` mark the house fonts carry or a stroke style, always rides with red and green | `panel` | A + P | `too-many-hues`, `hue-only-pass-fail`, `uncovered-glyph` |
