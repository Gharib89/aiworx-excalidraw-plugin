...
// the driver seam, so they share one browser: a session measures, gates and
// writes each diagram exactly as a single-shot call does (---- 10 ---- proves it).
await withAuthoring(async (author) => {
  // ---- 2. malformed skeletons are rejected, named, nothing written ----
  {
    ...
    const r = await rejectsWith("SkeletonError", author({ out, build: async () => [] }));
  ...
  }
  {
    ...
    const r = await rejectsWith("SkeletonError", author({ out, build: async () => undefined }));
  ...
  }
  {
    ...
    const r = await rejectsWith("SkeletonError", author({ out, build: async () => "elements" }));
  ...
  // ---- 3. the gate runs in-process: a defective build writes nothing ----
  {
    ...
    const r = await rejectsWith("GateError", author({
      out,
      build: async ({ PROSE }) => [
...
// stays single-shot: the default-driver launch path has to keep being exercised
...
// the same session treatment: these are layout claims, not launch claims
await withAuthoring(async (author) => {
...
if (process.platform === "win32" || process.getuid?.() === 0) {
  ...
  // one session for the pair: what is under test is the write, not the launch
  await withAuthoring(async (author) => {
    const dir = join(outDir, "readonly");
    const out = join(dir, "pair.excalidraw");
    const svgOut = out.replace(/\.excalidraw$/, ".svg");
    await author({ out, build: oneLine("first pass") });
    const before = readFileSync(out, "utf8");
    rmSync(svgOut);
    chmodSync(dir, 0o555);
    const r = await rejectsWith("Error", author({ out, build: oneLine("second pass") }));
    chmodSync(dir, 0o755);
...
// single-shot too, for the same reason — the plain authorDiagram path stays covered
{
...
// the register and options cases share a session too
await withAuthoring(async (author) => {
  ...
  // a typo in the register is a SkeletonError, not a silently ignored setting
  {
    ...
    const bad = await rejectsWith("SkeletonError", author({
      out,
      build: async () => [{ type: "rectangle", x: 0, y: 0, width: 10, height: 10 }],
    ...
    let built = false;
    await rejectsWith("SkeletonError", author({
      out,
...
// single-shot as well: three plain calls keep the default launch path covered
{
...
// graph() and fanOut() share a session
await withAuthoring(async (author) => {
...
// the splice and preset cases share the last session
await withAuthoring(async (author) => {
  ...
  // and gates, and that the default still refuses.
  {
    ...
    const kept = await rejectsWith("GateError", author({
  ...
  // ---- 16. output presets: one build, two surfaces, two type ramps ----
  {
    ...
    const shot = async (preset) => {
      ...
      const r = await author({ out, build, preset });
    ...
    await author({ out: omitted, build, svg: false });
    await author({ out: named, build, preset: "fit", svg: false });
    ...
    const bad = await rejectsWith("SkeletonError",
      author({ out: unknown, build, preset: "slide-4x3" }));
    ...
    const notAName = await rejectsWith("SkeletonError",
      author({ out: join(outDir, "preset-nonstring.excalidraw"), build, preset: 16 }));
